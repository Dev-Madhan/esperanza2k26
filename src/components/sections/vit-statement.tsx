...
const VitStatement = () => {
  ...
  return (
    <section ref={sectionRef} className="flex w-full flex-col items-center justify-center space-y-4 md:space-y-6 overflow-hidden bg-[#131313] py-12 md:py-20 pt-16 md:pt-32">
      <motion.button
        initial={{ opacity: 0, scale: 0.7, filter: "blur(20px)" }}
        animate={isInView ? { opacity: 1, scale: 1, filter: "blur(0px)" } : {}}
        transition={{ duration: 1, ease: [0.25, 0.1, 0.25, 1] }}
        whileHover={{ scale: 1.05 }}
        className="mt-4 md:mt-10 lg:mt-0 animate-shimmer rounded-full border-2 border-slate-400 bg-[linear-gradient(110deg,#000103,45%,#3B1344,55%,#000103)] bg-[length:200%_100%] px-6 py-3 md:px-20 md:py-6 lg:px-40 lg:py-10 font-power text-3xl md:text-6xl lg:text-9xl font-black text-slate-400 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50"
      ...
      >
        <motion.p
          ...
          className="heading-1 transform text-lg md:text-4xl lg:text-6xl xl:text-8xl font-black tracking-tight text-[#29B463]"
        >
          A PLACE TO LEARN
        </motion.p>
        <motion.p
          ...
          className="heading-2 transform text-lg md:text-4xl lg:text-6xl xl:text-8xl font-black tracking-tight text-[#FFC300]"
        >
          A CHANCE TO GROW
        </motion.p>
      </motion.div>
      ...
      <div ref={contentRef} className="space-y-3 md:space-y-4 px-4 md:px-8 font-power max-w-5xl mx-auto">
        {contentLines.map((line, lineIndex) => (
          <motion.div
            ...
            className="content-line flex flex-wrap items-center justify-center gap-x-1 gap-y-1 md:gap-y-2 text-[11px] md:text-lg lg:text-2xl text-center"
          >
